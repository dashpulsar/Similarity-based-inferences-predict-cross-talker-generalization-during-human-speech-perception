...
def process_single_layer_v4(layer_key, df_final, audio_dir, h5_path, tau=2):
    ...
    # 局部函数：在 Worker 内按需读取单层数据，避免通过 Joblib IPC 传输巨大的高维数组
    ...
    try:
        ...
        return f"EXCEPTION in {layer_key}: {traceback.format_exc()}"
        return None

def run_analysis_pipeline_v4(layers_list, df_final, audio_dir, h5_path, n_jobs=-1):
    print(f"Starting analysis for {len(layers_list)} layers...")
    start = time.time()
    
    # 关键修改：只传递 layer_key 和 h5_path 字符串，避免巨大的 NumPy 数组通过 loky 序列化传输
...
# 2. 获取所有的 Layer 名称
h5_path = "../preprocessing/xie21_features.h5"
h5_path = "../preprocessing/xie21_tsne_3d.h5"
with h5py.File(h5_path, 'r') as h5f:
    first_spk = list(h5f.keys())[0]
    first_sent = list(h5f[first_spk].keys())[0]
    layers_list = list(h5f[first_spk][first_sent].keys())

print(f"Found {len(layers_list)} layers to process.")

# 3. 运行 GLMM 主流线 
# 注意：强烈建议 n_jobs 不要超过 6，最多 8。
# 原因：1024维度的特征进行DTW计算本身占用内存，且每个 Worker 都要启动 R (lme4) 并发运行。
# n_jobs=24 极大概率会导致内存溢出 (OOM) 或 R 环境初始化锁死，从而报 TerminatedWorkerError。
print("Running HuBERT Layer Analysis...")
all_layers_results_hubert, df_final_hubert = run_analysis_pipeline_v4(layers_list, df_final, audio_dir, h5_path, n_jobs=24)
