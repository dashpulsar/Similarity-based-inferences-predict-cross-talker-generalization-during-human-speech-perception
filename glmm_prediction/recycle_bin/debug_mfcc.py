...
print("Running HuBERT Layer Analysis...")
all_layers_results_hubert_cos_full_dim, df_final_hubert_full_dim = run_analysis_pipeline_v4(layers_list, df_final, audio_dir, h5_path, n_jobs=-1)

all_layers_results_hubert_cos_full_dim.to_csv("xie21_glmm_results_hubert_cos_full_dim.csv", index=False)
...
# 为了测试绘图，我们这里先仅仅画 HuBERT 的结果
df_viz_combined = all_layers_results_hubert_cos_full_dim.copy()
df_corr = df_viz_combined[df_viz_combined['type'] == 'corrected'].copy()
...
print('Running MFCC extraction...')
mfcc_data = extract_mfcc_features(audio_dir)

print('Running process_memory_layer synchronously...')
try:
    std_data = standardization(mfcc_data)
    print('Standardization complete.')
    df_dist = precompute_layer_distances(df_final, audio_dir, std_data, 2, 'MFCC', func="mean")
    print('Distances computed. Shape:', df_dist.shape)
    if len(df_dist) == 0:
        print('Empty df_dist! Stopping.')
    else:
