...
def inputs_from_batches():
    ...
    for batch in ["train_test", "tt_all", "hve_tt", "hve_global"]:
        manifest = UPDATE / batch / "run_inputs.json"
        ...
        for original in json.loads(manifest.read_text(encoding="utf-8"))["inputs"]:
            ...
            source["model_dir"] = directory.relative_to(UPDATE).as_posix()
...
def validate():
    ...
    if [r["sha256"] for r in build["source_manifests"]] != [sha256_file(manifest)]:
        raise ValueError("Ratio outputs were not built from the current final manifest")
