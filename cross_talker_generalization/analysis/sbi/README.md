...
The [comparison figure](../model_comparison/conditional_ceiling/conditional_ceiling_comparison.png) connects the old and revised loss within each fold. It is a calculation diagnostic, with separate y-axis ranges by dataset, not a replacement for the SBI z-value display. PNG, PDF and SVG versions are saved alongside the [numerical comparison](../model_comparison/conditional_ceiling/comparison.csv), [coverage table](../model_comparison/conditional_ceiling/coverage_by_condition_fold.csv), and per-dataset predictions and source records.
