...
script_file <- normalizePath(sub("--file=", "", script_arg), mustWork = FALSE)
script_dir <- if (length(script_file) == 1 && nzchar(script_file)) dirname(script_file) else getwd()
git_root <- tryCatch(
  suppressWarnings(system2("git", c("rev-parse", "--show-toplevel"), stdout = TRUE, stderr = FALSE)),
  error = function(e) character(0)
)
output_dir <- if (length(git_root) == 1 && nzchar(git_root)) {
  file.path(git_root, "figures/main")
} else {
  script_dir
}
