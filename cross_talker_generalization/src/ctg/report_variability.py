...
def _plot_core_oof(oof: pd.DataFrame, destination: Path) -> None:
    ...
    figure.text(
        0.5,
        0.008,
        "B23 is absent because its identified exposure variability is confounded with condition, so an incremental OOF model is not identifiable.",
        ha="center",
        fontsize=8.5,
        color="#7f1d1d",
    )
...
1. `figure_03a_variability_true_oof_core_profiles`: primary result using participant-held-out OOF log-loss gain. B23 has no identifiable incremental OOF model because its identifiable exposure variability is confounded with condition.
...
- It shows 6 AN19, 16 X21, and 14 B23 computed methods.
