"""Expanded training-only tau/k grid, with bounded parallel R processes."""
import argparse
...
def main():
    ...
    p.add_argument('--dataset',choices=['AN19','X21'],default='X21')
    ...
    probe=Probe(out/'preparation',args.rscript,args.budget_seconds,4,dataset=args.dataset)
    tasks=[]
    for index,tau in enumerate(taus):
        path,_=probe.distances(float(tau))
        tasks.append((index,float(tau),path))
    manifest=dict(probe.manifest)
    manifest.update(scope=f'Expanded {args.dataset} base Tr-24 training landscape; fold 0 excluded',dataset=args.dataset,
        ...
        representation_of_similarity='standardized exp(-k*(d-min(d))); use expm1 when exponent span < 0.1; affine-equivalent to standardized exp(-k*d)',
        model=('binomial-logit: similarity_z + test_talker_id + (1|participant_id) + (1|analysis_item_id)' if args.dataset=='X21' else probe.manifest['model']),
