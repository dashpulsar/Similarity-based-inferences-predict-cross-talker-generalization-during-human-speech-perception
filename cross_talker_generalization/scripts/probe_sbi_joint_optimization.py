...
class Probe:
    def __init__(self, out, rscript, budget, workers, dataset='AN19'):
        ...
        self.input_path = derived/f'{dataset}-{dataset}_hubert_base_tsne-confirmatory-tr24-20260906-model-input.csv'
        ...
        cells_path = derived/f'{dataset}-pairs/cells.csv'
        pairs_path = derived/f'{dataset}-pairs/pairs.csv'
        ...
        # Preserve the registered word/keyword interval mapping for each dataset.
        ...
        spec = load_project(PROJECT/'configs/project.json').store(f'{dataset}_hubert_base_tsne')
        ...
        def crop(r, side):
            a=features[(getattr(r,f'{side}_speaker_id'),getattr(r,f'{side}_unit_id'))]
            bounds=[getattr(r,f'{side}_{field}_seconds') for field in ['start','end','duration']]
            bounds=[None if pd.isna(x) else float(x) for x in bounds]
            return np.ascontiguousarray(slice_by_time(a,*bounds))
        self.sequences = [(crop(r,'test'),crop(r,'source')) for r in self.pairs.itertuples()]
        # Include compilation/startup time separately, outside per-tau timings.
        dtw_distance(*self.sequences[0], tau=2.)
        self.setup_seconds = time.perf_counter()-t0
        self.manifest = dict(**runtime_record(), scope=f'Feasibility only: {dataset} base 3-D t-SNE Tr-24; folds 1+2 training; fold 0 excluded',
